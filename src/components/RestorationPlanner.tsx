...
export const FlowIntegration: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = FlowService.subscribeToUser((user) => {
      console.log('Flow user updated:', user);
      setFlowUser(user);
      setConnectionError('');
  ...
  const handlePrivyWalletLogin = async () => {
    setLoading(true);
    setConnectionError('');
    
    try {
      await privyLogin();
      setTransactionStatus('Successfully connected via Privy! Note: Flow features require Flow wallet.');
      setTimeout(() => setTransactionStatus(''), 5000);
    } catch (error) {
      console.error('Privy login error:', error);
      setConnectionError(error instanceof Error ? error.message : 'Failed to connect wallet via Privy');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleTestSignature = async () => {
    ...
    setLoading(true);
    try {
      const testMessage = `RegenReef signature test - ${new Date().toISOString()}`;
      ...
      setConnectionError(error instanceof Error ? error.message : 'Signature test failed');
    } finally {
      setLoading(false);
    }
  };
      ...
      {networkInfo && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <span className="text-blue-800 font-medium">Flow Network Status</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-blue-700">Access Node: Active</span>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900">Wallet Connected via Privy</h3>
                <p className="text-gray-600">{privyUser.displayName}</p>
                {privyUser.wallet && (
                  <p className="text-sm text-blue-600 font-mono">{privyUser.wallet}</p>
                )}
                <p className="text-xs text-orange-600 mt-1">
                  Note: Flow features require Flow wallet connection
                </p>
              </div>
            </div>
      ...
      {!flowUser?.loggedIn ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
          <div className="text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">Connect to Flow Testnet</h3>
            <p className="text-gray-600 mb-6">
              Connect your Flow wallet to testnet for AI agent management with Flow signatures
            </p>
        ...
        <>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              {/* Direct Flow Wallet Connection */}
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl">
                <h4 className="font-semibold text-blue-900 mb-2">Flow Wallet (Recommended)</h4>
                <p className="text-sm text-blue-700 mb-4">
                  Connect directly to Flow testnet using Flow-native wallets with signature support
                </p>
                <button
                  onClick={handleFlowLogin}
                  disabled={loading}
              ...
              <div className="p-4 bg-green-50 border border-green-200 rounded-xl">
                <h4 className="font-semibold text-green-900 mb-2">Alternative: Privy Wallet</h4>
                <p className="text-sm text-green-700 mb-4">
                  Connect using Privy (limited Flow features, no signatures)
                </p>
                <button
                  onClick={handlePrivyWalletLogin}
                  disabled={loading || privyAuthenticated}
                ...
                >
                  ...
                  <span>
                    {loading ? 'Connecting...' : 
                     privyAuthenticated ? 'Connected via Privy' : 
                     'Connect via Privy'}
                  </span>
                </button>
              </div>
            ...
            {signatureTest && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-center space-x-2 mb-2">
                  ...
                  <span className="text-green-800 font-medium">Signature Test Successful</span>
                </div>
                <div className="space-y-2 text-sm">
                  ...
                  <div>
                    ...
                    <span className="font-mono text-gray-800 break-all">{signatureTest.signature}</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-3 bg-blue-50 rounded-lg">
                      <div className="flex items-center space-x-2 mb-1">
                        ...
                        <span className="text-sm font-medium text-blue-900">Flow Signature Required</span>
                      </div>
                      <p className="text-xs text-blue-700">
                        This action will require a Flow signature to verify your identity and create the agent on-chain.
                      </p>
                    </div>
                  </div>
            ...
            {loading && agents.length === 0 ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-600 mb-2">No agents created yet</p>
                <p className="text-sm text-gray-500">Create your first AI agent on the Flow blockchain with signature verification</p>
              </div>
            ) : (
              <div className="space-y-4">
                {agents.map((agent) => (
                  <div key={agent.id} className="p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <h4 className="font-medium text-gray-900">Agent #{agent.id}</h4>
